...
import co.com.pragma.apiclients.configuration.constant.PatternConstant;
...
public class GlobalHelper implements Serializable {
   private static final long serialVersionUID = -2804865259683967248L;
   private final EntityManager entityManager;
   private final GenericTypeHelper genericType;
   private final ClientHelper client;

   @Lazy
   public GlobalHelper(EntityManager entityManager,
                       GenericTypeHelper genericType,
                       ClientHelper client) {
   ...
   public <T> JPAQuery<T> query() {
      return new JPAQuery<T>(this.entityManager);
   }

   public <T, ID> T getEntity(ID id, String mensajeError, JpaRepository<T, ID> repository) throws GlobalException {
      var entidad = repository.findById(id).orElse(null);
      if (entidad != null) return entidad;
      throw new GlobalException(mensajeError);
   }

   public <T> T getApiEntity(Integer id, String mensajeError, Response<T> respuesta) throws GlobalException {
      if (Boolean.TRUE.equals(respuesta.getFlag())) return respuesta.getEntity();
      throw new GlobalException(mensajeError, Response.builder().message(mensajeError).build());
   }

   public <T> T getApiEntity(Integer id, Response<T> respuesta) {
      if (Boolean.TRUE.equals(respuesta.getFlag())) return respuesta.getEntity();
      return null;
   }
